...
def generate_ellipse_path(R_, T_, n_frames=600, const_speed=True, z_variation=0., z_phase=0.):
    poses = []
    for i in range(len(R_)):
        R = R_[i]
        T = T_[i]
        tmp_view = np.eye(4)
        tmp_view[:3] = np.concatenate([R.T, T[:, None]], 1)
    ...
    def get_positions(theta):
        ...
        scaling_factor = 1
        return np.stack([
            (low[0] + (high - low)[0] * (np.cos(theta) * .5 + .5))* scaling_factor,
            (low[1] + (high - low)[1] * (np.sin(theta) * .5 + .5))* scaling_factor,
