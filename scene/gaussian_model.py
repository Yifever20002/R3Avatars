...
from nets.gaussian_decoder import GaussianDecoder
...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, smpl_type : str, motion_offset_flag : bool, actor_gender: str):
        ...
        self.multiscale_res_multipliers = [1,2,4]
        self.time_dim = 32
        self.feature_dim = 0
        self.grid_config = [{
                'grid_dimensions': 2,
                'input_coordinate_dim': 4,
                'output_coordinate_dim': self.time_dim,
        ...
        self.grids.to('cuda')
        print("Initialized model grids:", self.grids)

        # d_xyz opacity scales rotations
        self.gaussian_decoder = GaussianDecoder(input_ch=self.feature_dim).to('cuda')
    ...
    def restore(self, model_args, training_args):
        ...
        self.grids,
        self.feature_dim,
        self.gaussian_decoder) = model_args
        self.training_setup(training_args)
        self.xyz_gradient_accum = xyz_gradient_accum
        self.denom = denom
        self.optimizer.load_state_dict(opt_dict)

    def restore_test(self, model_args):
        (self.active_sh_degree, 
        self._xyz, 
        self._features_dc, 
        self._features_rest,
    ...
    def get_covariance(self, scales, rotations, scaling_modifier = 1, transform=None):
        return self.covariance_activation(scales, scaling_modifier, rotations, transform)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        # define canonical bounding box
        min_corner = torch.min(self._xyz, dim=0).values
        max_corner = torch.max(self._xyz, dim=0).values
        aabb = torch.stack([min_corner-0.1, max_corner+0.1])
    ...
    def training_setup(self, training_args):
        ...
        if not self.motion_offset_flag:
            ...
            grid_params = [param for grid in self.grids for param in grid.parameters()]
            ...
            l = [
                ...
                # {'params': [self._scaling], 'lr': training_args.scaling_lr, "name": "scaling"},
                # {'params': [self._rotation], 'lr': training_args.rotation_lr, "name": "rotation"},
                {'params': grid_params, 'lr': training_args.position_lr_init, "name": "grids"},
                ...
                {'params': self.gaussian_decoder.parameters(), 'lr': training_args.lbs_offset_lr, "name": "gaussians_decoder"},
            ] 
    ...
    def get_bweights(self, query_pts, t_vertices, lbs_weights):
        ...
        if lbs_weights is None:
            ...
            # bweights = self.SMPL_NEUTRAL['weights'].unsqueeze(0)
