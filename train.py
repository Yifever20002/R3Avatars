...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, \
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if viewpoint_cam.dataset_type == 'hifi4g':
            mask_loss = torch.zeros_like(Ll1).cuda()
        else:
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    if len(gaussians._xyz) > 35000:
                        print('points over 35000! set threshold: ', 2+(len(gaussians._xyz)-35000)/5000)
                        densify_grad_threshold = (2+(len(gaussians._xyz)-35000)/5000)*opt.densify_grad_threshold
...
def training_report(tb_writer, iteration, loss_item, elapsed, testing_iterations,
                    scene : Scene, renderFunc, renderArgs):

    Ll1, loss, l1_loss = loss_item
    log_loss = {
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[i for i in range(0, 60_001, 2000)])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[i for i in range(0, 60_001, 10000)])
